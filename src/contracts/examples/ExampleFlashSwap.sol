...
contract ExampleFlashSwap is IUniswapV2Callee {
    ...
    function uniswapV2Call(address sender, uint256 amount0, uint256 amount1, bytes calldata data) external override {
        ...
        uint256 amountETH;
        {
            ...
            address token0 = IFraxswapPair(msg.sender).token0();
            address token1 = IFraxswapPair(msg.sender).token1();
            assert(msg.sender == FraxswapRouterLibrary.pairFor(factory, token0, token1)); // ensure that msg.sender is actually a V2 pair
        ...
        if (amountToken > 0) {
            ...
            uint256 amountRequired = FraxswapRouterLibrary.getAmountsIn(factory, amountToken, path)[0];
            ...
            uint256 amountRequired = FraxswapRouterLibrary.getAmountsIn(factory, amountETH, path)[0];
