...
library FraxswapRouterLibrary {
    ...
    function getAmountInU112Fixed(
        ...
        (uint112 _reserve0, uint112 _reserve1, ) = pair.getReserves();
        (uint112 _reserveInFlipped, uint112 _reserveOutFlipped) = _tokenOut == pair.token0()
            ? (_reserve0, _reserve1)
            : (_reserve1, _reserve0);
        require(_amountOut > 0 && _reserveInFlipped > 0 && _reserveOutFlipped > 0); // INSUFFICIENT_OUTPUT_AMOUNT, INSUFFICIENT_LIQUIDITY
        uint256 numerator = uint256(_reserveInFlipped) * _amountOut * 10_000;
        uint256 denominator = (uint256(_reserveOutFlipped) - _amountOut) * pair.fee();
        return (numerator / denominator) + 1;
    }

    // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    function getAmountOutU112Fixed(
        address _pairAddress,
        uint256 _amountIn,
        address _tokenIn
    ) internal view returns (uint256) {
        IFraxswapPair pair = IFraxswapPair(_pairAddress);
        (uint112 _reserve0, uint112 _reserve1, ) = pair.getReserves();
        (uint112 _reserveIn, uint112 _reserveOut) = _tokenIn == pair.token0()
            ? (_reserve0, _reserve1)
            : (_reserve1, _reserve0);
        ...
        uint256 denominator = (uint256(_reserveIn) * 10_000) + amountInWithFee;
